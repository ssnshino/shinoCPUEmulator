...
function buildData(){
 ...
 return {version:'0.1',baseline:'BIOS/MON v0.3 + S80B v2 + CBIOS/WBOOT v0.2 + CP/M 2.2 filesystem v0.1 candidates',revision:'feature/shino80-cpm-filesystem-v01-20260926',families,instructions,
  ...
  machine:{romSize:rom.ROM_SIZE,bootRomSize:memory.BOOT_ROM_SIZE,extensionRomSize:memory.EXTENSION_ROM_SIZE,memoryControlPort:memory.MEMORY_CONTROL_PORT,vramBase:video.TEXT_VRAM_BASE,vramCells:video.CELL_COUNT,width:video.SCREEN_W,height:video.SCREEN_H,cols:video.COLS,rows:video.ROWS,
   ...
   systemDiskMagic:systemDisk.SYSTEM_DISK_MAGIC,systemDiskVersion:systemDisk.SYSTEM_DISK_VERSION,systemDiskEntry:systemDisk.SYSTEM_ENTRY,systemDiskPayloadBytes:diskImage.payload.length,systemDiskHeader:Array.from(diskImage.header.slice(0,17)),systemDiskFiles:diskImage.files.map(file=>({name:file.name,size:file.size,records:file.records,blocks:Array.from(file.blocks)})),
   cpmCcpOrigin:cpm22.CPM22_CCP_ORIGIN,cpmBdosOrigin:cpm22.CPM22_BDOS_ORIGIN,cpmBdosEntry:cpm22.CPM22_BDOS_ENTRY,cpmCcpHash:cpm22.CPM22_CCP_SHA256,cpmBdosHash:cpm22.CPM22_BDOS_SHA256,
