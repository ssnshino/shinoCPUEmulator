...
const [family,cache,output,policy='full']=process.argv.slice(2);
if(!['full','documented'].includes(policy))throw Error('Policy must be full or documented');
const flagMask=policy==='full'?0xFF:0xD7;
if(!['cb','base','ed','dd','fd','ddcb','fdcb'].includes(family)||!cache||!output)throw Error('Usage: node scripts/check-z80-oracle.cjs cb|base|ed|dd|fd|ddcb|fdcb CACHE_DIR OUTPUT_JSON');
// ED inventory is independent of our decoder: pinned upstream contains 80 files.
const indexedCB=family==='ddcb'||family==='fdcb';
const names=Array.from({length:256},(_,i)=>i).filter(i=>indexedCB||family==='cb'||(family==='ed'?((i>=0x40&&i<=0x7F)||[0xA0,0xA1,0xA2,0xA3,0xA8,0xA9,0xAA,0xAB,0xB0,0xB1,0xB2,0xB3,0xB8,0xB9,0xBA,0xBB].includes(i)):![0xCB,0xDD,0xED,0xFD].includes(i))).map(i=>(indexedCB?family.slice(0,2)+' cb __ ':family==='base'?'':family+' ')+i.toString(16).padStart(2,'0'));
async function main(){
 ...
 const summary={revision,family,sourceHashes,flagMask:flagMask.toString(16).toUpperCase(),excluded:[...(policy==='full'?[]:['WZ','P','Q','X/Y']),'pin waveforms','interrupt dispatch'],opcodes:[],cases:0,pass:0,failures:0,details:[]};
 ...
 for(const name of names){
  ...
  for(const [index,test] of cases.entries()){
   ...
   for(const k of ['wz','q','p'])cpu.state[k]=initial[k]??0;
   ...
   if(policy==='full')for(const k of ['wz','p','q'])if(actual[k]!==expected[k])mismatch.push([k,expected[k],actual[k]]);
   for(const k of fields)if(actual[k]!==expected[k])mismatch.push([k,expected[k],actual[k]]);
   if((actual.f&flagMask)!==(expected.f&flagMask))mismatch.push(['F&'+summary.flagMask,expected.f&flagMask,actual.f&flagMask]);
   for(const [prefix,keys] of [['af_',['aAlt','fAlt']],['bc_',['bAlt','cAlt']],['de_',['dAlt','eAlt']],['hl_',['hAlt','lAlt']]]){const mask=prefix==='af_'?0xFF00|flagMask:65535,v=(actual[keys[0]]<<8)|actual[keys[1]];if((v&mask)!==(expected[prefix]&mask))mismatch.push([prefix,expected[prefix]&mask,v&mask]);}
