...
function machine(traceLimit=512){
  ...
  const bus=new Shino80Bus({traceLimit,romRanges:[[0,0x1FFF]],ioDevices:[keyboard]});
  bus.load(rom.bytes,0);const cpu=new Z80Core(bus);cpu.reset();cpu.runInstructions(rom.meta.instructionsBeforeLoop);
...
function invoke(m,address,hl){m.bus.load([0xCD,address&255,address>>8,0x76],0x3000);Object.assign(m.cpu.state,{pc:0x3000,sp:0xF000,h:hl>>8,l:hl&255,halted:false});until(m.cpu,()=>m.cpu.state.halted);}
...
function expectedMnemonic(d,bytes,address=0x2000){
...
function directDisassembler(){
  const bus=new Shino80Bus({traceLimit:0,romRanges:[[0,0x1FFF]]});bus.load(rom.bytes,0);bus.load([0xCD,0x1B,0x01,0x76],0x3000);const cpu=new Z80Core(bus);
  return (bytes,address=0x2000)=>{
    ...
    Object.assign(cpu.state,{pc:0x3000,sp:0xF000,h:address>>8,l:address&255,halted:false});until(cpu,()=>cpu.state.halted,20000);
...
for(const [family,decode] of Object.entries(familyDecoders))for(let opcode=0;opcode<256;opcode++){
  ...
  assert.equal(actual.length,bytes.length,`${family} length ${opcode.toString(16)}`);assert.equal(actual.next,(0x2000+bytes.length)&0xFFFF);encodings++;
}
assert.equal(encodings,1780);
for(const [bytes,fragment,next] of [
  [[0xDD,0xFD,0x21,0x34,0x12],'LD IY,1234h',0x2005],
  [[0xDD,0xED,0x44],'NEG',0x2003],
  [[0xDD,0xDD,0xDD,0xDD,0xDD,0x00],'DB DDh ; PREFIX LIMIT',0x2001]
...
{
  const m=machine();for(let i=0;i<0x110;i++)m.bus.debugPoke(0x2000+i,i);command(m,'D 2000 2003');assert(screen(m).includes('2000: 00 01 02 03'));
}
{
...
{
  const m=machine(0);let reads=[];const read=m.bus.cpuRead.bind(m.bus);m.bus.cpuRead=(address,options)=>{if(address>=0x2000&&address<=0x2200)reads.push(address);return read(address,options);};
  command(m,'D 2000 2200');assert.equal(reads.filter(a=>a>=0x2000&&a<=0x20FF).length,256);assert(!reads.includes(0x2100));assert(screen(m).includes('RANGE LIMITED'));
}
...
{
  const m=machine();m.bus.load([0x3E,0x41,0xDD,0x36,0xFE,0x55,0xC9],0x2000);command(m,'U 2000 2004');const output=screen(m);
  assert(output.includes('2000: 3E 41  LD A,41h'));assert(output.includes('2002: DD 36 FE 55  LD (IX-02h),55h'));assert(!output.includes('2006: C9'));
}
{
...
{
  const m=machine();for(let i=0;i<0x210;i++)m.bus.debugPoke(0x2000+i,0);command(m,'U 2000 2200');assert(screen(m).includes('RANGE LIMITED'));
}
