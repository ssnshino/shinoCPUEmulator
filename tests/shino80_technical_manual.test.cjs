...
assert.equal(data.machine.beeperPort,0x40);
...
assert.equal(data.machine.systemDiskMagic,'S80B');assert.equal(data.machine.systemDiskVersion,2);assert.equal(data.machine.systemDiskEntry,0x8000);assert.equal(data.machine.systemDiskHeader.length,17);
assert.equal(data.machine.cbiosBytes,645);assert.equal(data.machine.cbiosEnd,0xFC85);
...
for(const match of html.matchAll(/<link\b[^>]*\bhref=["']?([^"' >]+)/gi))assert(match[1].startsWith('data:'),'standalone link asset');
