(function(){
  ...
  const devices=[
    ...
    {id:'disk-a',icon:'A:',name:'VIRTUAL DISK A',desc:'SYSTEM v1 · 77×26×128 / I/O 30h–36h',state:'ONLINE'},
  ...
  function renderInspector(){
    ...
    if(ui.view==='devices'){
      ...
      else if(d.id==='disk-a')html+=`<div class="inspector-section"><div class="inspector-kv"><span>Media</span><b>${systemDisk.meta.magic} v${systemDisk.meta.version} SYSTEM</b></div><div class="inspector-kv"><span>Geometry</span><b>77 TRACKS × 26 SECTORS</b></div><div class="inspector-kv"><span>Sector</span><b>128 BYTES</b></div><div class="inspector-kv"><span>Image</span><b>256,256 BYTES</b></div><div class="inspector-kv"><span>Ports</span><b>30h–36h</b></div><div class="inspector-kv"><span>Selection</span><b>A:${diskA.track}/${diskA.sector}</b></div><div class="inspector-kv"><span>Transfer</span><b>${diskA.transferMode?diskA.transferMode.toUpperCase()+' '+diskA.transferRemaining+' B':'IDLE'}</b></div><div class="inspector-kv"><span>Write protect</span><b>${diskA.writeProtected?'ON':'OFF'}</b></div><div class="inspector-kv"><span>Error</span><b>${diskA.error}</b></div></div><div class="inspector-section inspector-note">MON O reads the original system image into RAM + CBIOS. Page zero JP FA03h provides disk-backed WBOOT. Media stays mounted across RESET and POWER; this is not CP/M yet.</div>`;
