(function(){
  ...
  function updateRegisters(){
    ...
    queryOne('#displayPc').textContent=hex(s.pc,4);queryOne('#displayR').textContent=hex(s.r,2);queryOne('#displayT').textContent=String(s.tStates);
  ...
  function renderInspector(){
    ...
    if(ui.view==='cpu')html+=`<div class="inspector-section"><div class="inspector-kv"><span>Current opcode</span><b>00h NOP</b></div><div class="inspector-kv"><span>Accuracy</span><b>Level 1</b></div><div class="inspector-kv"><span>Bus trace</span><b>M-cycle abstract</b></div></div><div class="inspector-section inspector-note">The original Altair-inspired panel survives here as the CPU Debug Lab.</div>`;
  ...
  function render(){updateRegisters();updateBusLeds();updateSignalLamps();updateTrace();updateMemory();renderInspector();queryOne('#machineState').textContent=ui.running?'RUNNING':'READY';queryOne('#statusRun').textContent=ui.running?'RUNNING':'READY';queryOne('#runPauseLabel').textContent=ui.running?'PAUSE':'RUN';queryOne('#runPauseBtn').classList.toggle('running',ui.running);queryOne('#paceLabel').textContent=paces[ui.paceIndex].label;queryOne('#tracePanel').classList.toggle('closed',!ui.traceOpen);queryOne('#tracePanel').classList.toggle('open',ui.traceOpen);queryOne('#traceToggleBtn').textContent=ui.traceOpen?'▲':'▼';ui.dirty=false;}
  ...
  function bind(){queryAll('[data-view]').forEach(b=>b.addEventListener('click',()=>setView(b.dataset.view)));queryOne('#runPauseBtn').addEventListener('click',toggleRun);queryOne('#stepBtn').addEventListener('click',stepOne);queryOne('#paceBtn').addEventListener('click',cyclePace);queryOne('#resetBtn').addEventListener('click',resetCpu);queryOne('#moreBtn').addEventListener('click',()=>{const action=prompt('SHINO-80 MORE\n1: RESET\n2: BURST ×256\n3: CLEAR TRACE','2');if(action==='1')resetCpu();if(action==='2')stepBatch(256);if(action==='3'){bus.clearTrace();ui.dirty=true;}});queryOne('#burstBtn').addEventListener('click',()=>stepBatch(256));queryOne('#clearTraceBtn').addEventListener('click',()=>{bus.clearTrace();ui.dirty=true;});queryOne('#traceToggleBtn').addEventListener('click',toggleTrace);addEventListener('resize',()=>{ui.dirty=true;});}
  ...
  buildStaticUi();bind();const ok=selfTest();queryOne('#selfTest').textContent=ok?'NOP SELF TEST PASS':'SELF TEST FAIL';setView('display');render();requestAnimationFrame(observerLoop);
})();
