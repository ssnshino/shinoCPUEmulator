(function(){
  ...
  const regGrid=queryOne('#regGrid');
  for(const [label,width,key] of regDefs){const row=createRegRow(label,width);row.dataset.key=key;regGrid.appendChild(row);}

  makeLeds(queryOne('#addrLeds'),16,'amber');
  makeLeds(queryOne('#dataLeds'),8,'green');

  function updateRegisters(){
    const s=cpu.state;
    for(const row of queryAll('#regGrid .regRow')){
    ...
    for(const e of queryAll('.flag')){
      e.querySelector('.led').classList.toggle('on',!!flags[e.dataset.flag]);
    }
    queryOne('#iff1').textContent=s.iff1?'1':'0';
    queryOne('#iff2').textContent=s.iff2?'1':'0';
    queryOne('#im').textContent=String(s.im);
    queryOne('#tstates').textContent=String(s.tStates).padStart(9,'0');
    queryOne('#icount').textContent=String(s.instructions).padStart(7,'0');
  }

  function updateInstruction(){
    const i=cpu.lastInstruction;
    if(!i){queryOne('#instPc').textContent='0000';queryOne('#instMnemonic').textContent='RESET';queryOne('#instOpcode').textContent='--';return;}
    queryOne('#instPc').textContent=hex(i.address,4);
    queryOne('#instMnemonic').textContent=i.mnemonic;
    queryOne('#instOpcode').textContent=hex(i.opcode,2);
  }
  ...
  function updateBus(){
    ...
    if(fetch){
      ...
      setLeds(queryOne('#addrLeds'),fetch.address,16);queryOne('#addrHex').textContent=hex(fetch.address,4);
      setLeds(queryOne('#dataLeds'),fetch.data,8);queryOne('#dataHex').textContent=hex(fetch.data,2);
    }
  }
  ...
  function pulseSignals(events){
    ...
    for(const e of queryAll('.signal'))e.querySelector('.led').classList.toggle('on',active.has(e.dataset.signal));
    clearTimeout(pulseTimer);
    pulseTimer=setTimeout(()=>{for(const e of queryAll('.signal .led'))e.classList.remove('on');},180);
  }

  function updateTrace(){
    const e=queryOne('#trace');e.innerHTML='';
  ...
  function updateMemory(){
    const root=queryOne('#memory');root.innerHTML='';
  ...
  function stepOne(){
    ...
    try{
      ...
      pulseSignals(events);
      queryOne('#runState').textContent=runningVisual?'VISUAL RUN':'READY';queryOne('#runState').style.color='var(--green)';
      render();
    }catch(err){
      stop();
      queryOne('#runState').textContent='FAULT';queryOne('#runState').style.color='var(--red)';
      queryOne('#fault').textContent=String(err.message||err);
    }
  }

  function run(){if(timer)return;runningVisual=true;queryOne('#runState').textContent='VISUAL RUN';timer=setInterval(stepOne,125);}
  function stop(){runningVisual=false;if(timer){clearInterval(timer);timer=null;}queryOne('#runState').textContent='READY';}
  function reset(){stop();cpu.reset();bus.clearTrace();lastFetchAddress=0;queryOne('#fault').textContent='';render();pulseSignals([{signals:['RESET']}]);}
  function burst(){
    stop();
    const before=bus.trace.length;
    try{
      ...
      queryOne('#runState').textContent='READY';queryOne('#runState').style.color='var(--green)';
      render();
    }catch(err){
      queryOne('#runState').textContent='FAULT';queryOne('#runState').style.color='var(--red)';
      queryOne('#fault').textContent=String(err.message||err);
  ...
  queryOne('#resetBtn').addEventListener('click',reset);
  queryOne('#stepBtn').addEventListener('click',stepOne);
  queryOne('#runBtn').addEventListener('click',run);
  queryOne('#pauseBtn').addEventListener('click',stop);
  queryOne('#burstBtn').addEventListener('click',burst);
  queryOne('#clearBtn').addEventListener('click',clearTrace);
  ...
  const ok=selfTest();queryOne('#selfTest').textContent=ok?'NOP SELF TEST PASS':'SELF TEST FAIL';queryOne('#selfTest').classList.toggle('warn',!ok);
  render();
})();
